...
//error_log ("qty check");
 ...
 $AvailQty=0;
        try {
           ...
           // $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "qtyavailable: " . $AvailQty . " gcAllQty: " . json_encode($gcAllQty));
                ...
                if ((!isset($gcAllQty["errordesc"]) || $gcAllQty["errordesc"] == "") ) {
                    foreach ($gcAllQty["ItemsWarehouseProductListResponseContainerItems"] as $item) {
                        if ((trim($whselist) == "") || (strpos(strtoupper($whselist), strtoupper($item["whse"])) !== false)) {
                            ...
                            $qtyAvailable['qty'] +=($item["qtyonhand"] - $item["qtyreservd"] - $item["qtycommit"]);
                            $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "qtyavailable: " . json_encode($qtyAvailable));
                    ...
                    if ((trim($whselist) == "") || (strpos(strtoupper($whselist), strtoupper($item["whse"])) !== false)) {
                            ...
                            $qtyAvailable['qty']=($item["qtyonhand"] - $item["qtyreservd"] - $item["qtycommit"]);
                            $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "qtyavailable2: " . json_encode($qtyAvailable));
                        }
                }
