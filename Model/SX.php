...
//$this->gwLogAPITime($result);
	...
	public function SalesCreditCardAuthInsert($cono, $orderno, $ordersuf, $amount, $authamt, $bankno, $cardno, $charmediaauth, $cmm, $commcd, $createdt, $currproc, $mediaauth, $mediacd, $origamt, $origproccd, $preauthno, $processcd, $processno, $respdt, $response, $saleamt, $statustype, $submitdt, $transcd, $transdt, $user1, $user2, $user3, $user4, $user5, $user6, $user7, $user8, $user9, $exp, $moduleName = "")
    {
        ...
        try {
            ...
            if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ) $params1->cardno = $cardno;
    ...
    public function SalesCustomerPricingSelect($cono, $prod, $whse, $custno, $shipto, $qty, $moduleName = "")
    {
        ...
        try {
			...
			if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ) $params1->operinit = $operinit;
            $params1->qty = $qty;
            $params1->APIKey = $apikey;
    ...
    public function SalesCustomerPricingList($cono, $custno, $whse, $shipto, $qty, $productlist, $moduleName = "")
    {
        ...
        try {
            ...
            $thisparam= array(
            'cono' => $cono, 'custno'=>$custno, 'whse'=>$whse,'shipto'=>$shipto,'qty'=>$qty,'APIKey'=>$apikey
            );
...
public function SalesCustomerSelect($cono, $custno, $moduleName = "")
    {
        ...
        $params1 = (object)[];
        $params1->cono = $cono;
        if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ) $params1->operinit = $this->getConfigValue('operinit');
        $params1->custno = $custno;
        $params1->APIKey = $apikey;
        $rootparams = (object)[];
...
public function SalesShipToList($cono, $custno, $moduleName = "")
    ...
    public function ItemsWarehouseProductSelect($cono, $brsprod, $brswhse, $brsstatustype, $moduleName = "")
    {
        ...
        $params1 = (object)[];
        $params1->cono = $cono;
if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ){
	 $params1->operinit = $this->getConfigValue('operinit');
    ...
    public function ItemsWarehouseProductList($cono, $brsprod, $brswhse = "", $moduleName = "")
    {
        ...
        if (strpos($this->getConfigValue('apiurl'),'csd') ===false  ){
            ...
            if (isset($brsprod)  && !empty($brsprod) &&  !is_array($brsprod)) {
                ...
                $thisparam=array_merge( $thisparam,array('brs-prod' => $brsprod));
            }
            ...
            $thisparamLines= array(   'ItemsWarehouseProductListProdRequestContainer' => $paramsDetail->getArrayCopy()               );
                ...
                } elseif (isset($brsprod) && is_array($brsprod)) {
            ...
            if ($brswhse != "") {
                $paramsHead[] = new SoapVar($brswhse, XSD_STRING, null, null, 'brs-whse' );
            }
            $paramsDetail[] = new SoapVar($prod, XSD_DECIMAL, null, null, 'prod' );
            $paramsHead->append(new SoapVar($paramsDetail, SOAP_ENC_OBJECT , null, null, 'ItemsWarehouseProductListProdRequestContainer' ));
...
if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ){
    ...
    public function SubmitOrder($invoice, $moduleName = "", $forceQuote = false)
    {
        $configs = $this->getConfigValue([
            'sxcustomerid', 'cono', 'whse', 'slsrepin', 'defaultterms', 'operinit', 'transtype','shipviaty','flatshipvia',
            'slsrepout', 'holdifover', 'shipto2erp', 'potermscode', 'orderaspo', 'sendtoerpinv', 'apikey','alloweditaddress','takenby','alertemail','taxtakenby'
        ]);
        extract($configs);
        if ($forceQuote) {
        ...
        if ($forceQuote) { //cartquickpro/sidebar/
            if (strpos($url, "cartquickpro/wishlist_index") !== false || strpos($url, "customer/account") !== false || strpos($url, "cartquickpro/sidebar") !== false || strpos($url, "cartquickpro/cart/add") !== false || strpos($url, "wishlist/index") !== false  || strpos($url, "sxorders/customer") !== false  || strpos($url, "carts/mine/estimate-shipping-methods") !== false  || strpos($url, "xxxcarts/mine/payment-information") !== false) { //carts/mine/payment-information
...
//$this->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "done checking shipto ");
       ...
       $this->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "url after street " . $url . "" . " Increment ID=" . $orderincid);
        ...
        $phoneno = '';
        $faxphoneno = '';
       if (isset($shipping_address["city"])) $city = $shipping_address["city"];
        $state = $statecd;
        if (isset($shipping_address["postcode"])) $zipcd = $shipping_address["postcode"];
        if (isset($shipping_address["country_id"])) $countrycd = $shipping_address["country_id"];
        if (isset($shipping_address["telephone"])) $phoneno = $shipping_address["telephone"];
        if (isset($shipping_address["fax"])) $faxphoneno = $shipping_address["fax"];  //$shipping_address[""];
        $pricetype = '';
        $poreqfl = '';
        $bofl = '';
        $subfl = '';
        ...
        $user8 = '';
        $user9 = '';
        if (isset($shipping_address["email"])) $email = $shipping_address["email"];
        $pricecd = '';
        $transproc = '';
        $addon1 = 0;
        $addon2 = 0;
...
if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ){
	$paramsShipTo->operinit = $operinit;
        ...
        $thisparam= array(
            'cono' => $cono ,'operinit' =>$operinit  ,'transtype' => $transtype ,'batchnm' =>$batchnm  ,'whse' => $whse ,'custno' =>$custno  ,'wodiscamt'  => $wodiscamt ,'wodiscpct'  => $wodiscpct , //,'slsrepin' => $slsrepin 
            'createsalesorderlinesyesno'  =>$createsalesorderlinesyesno  ,'user6' => $user6 , 'user7' => $user7 ,
            'APIKey' => $apikey , 'placedby'  => $placedby ,'takenby' => $takenby ,'custpo' =>$custpo  ,'enterdt' => $enterdt ,'transdt' =>  $transdt, 'terms' => $terms ,'printprice' =>$printprice  ,//'slsrepout' =>$slsrepout,
            'shipviaty'=>$shipviaty
            );
    ...
    if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ){
        ...
        $paramsTerms[] = new SoapVar($terms, XSD_DECIMAL, null, null, 'sxTermsCd' );
        ...
        foreach ($items as $item) {
        	...
        	$lineno=$lineno+1;
        	  try{
        //	   	$this->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "Getting SX Product 1");
...
//return false; //need to test address stuff
        ...
        if ($haslines ) {
            if (strpos($this->getConfigValue('apiurl'),'csd') !==false  ){
                ...
                //$this->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "Calling SalesOrderInsert");
               // if ($customerID==1 && !$forceQuote){
               //     $this->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "not inserting order to SX because custid=1 " . " Increment ID=" . $orderincid);
               // } else {
...
//$this->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , "SalesOrderInsert forcequote");
    ...
    {
        $configs = $this->getConfigValue([
            'sxcustomerid', 'cono', 'whse', 'slsrepin', 'defaultterms', 'operinit', 'transtype','shipviaty',
            'slsrepout', 'holdifover', 'shipto2erp', 'potermscode', 'orderaspo', 'sendtoerpinv', 'apikey'
        ]);
        extract($configs);
      ...
      /*  $paramsHead[] = new SoapVar($cono, XSD_DECIMAL, null, null, 'cono');
        ...
        $thisparam= array(
            'cono' => $cono ,'whse' => $whse ,'custno' =>$custno  ,'taxauthcountryCAUSorAU' => $countrycd ,'taxauthstate'  => $statecd ,
            'APIKey' => $apikey
            );
         if ($shipping_address["city"] != "") {
