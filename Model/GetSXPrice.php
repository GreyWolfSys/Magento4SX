...
class GetSXPrice implements ObserverInterface
{
    ...
    //    $debuggingflag = "false";
        ...
        foreach ($productsCollection as $product) {
                ...
                )////
                {
                    ...
                    /*if ( $customer_id>0){
                        ...
                        if(!empty($result)) {
                            ...
                            $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": Product $prod not found in db, new price not set");
                        }
                        //return $price;
                    } else {
                        //return $price;
            ...
            if (strpos($url, 'admin') !== false || strpos($url, '/catalog/product/index/key/') !== false || $admin == true || strpos($url, '/product/index/key') !== false || $pagestate->getAreaCode() == 'adminhtml') { //https://nee2go.com/cstore/rest/cstore/V1/carts/mine/totals-information
                ...
                if (strpos($url, 'checkout/cart/updateItemQty') !== false) {
                    ...
                    if (($currentTime - $oldTime) < 5) {
                        if ($debuggingflag == "true") {
                            $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": ", "Skipping SX price check for unneeded page & very recent update " . $url);
                            return "";
                        }
                    } else {
                        $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": ", "Running SX price on cart due to quantity update " . $url);
                    }
                ...
                if ($debuggingflag == "true") {
                    $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": ", "Skipping SX price check for invis2");
                }
            ...
            // $this->sx->gwLog(__CLASS__ . "/" . __FUNCTION__ . ": " , $debugstr);
